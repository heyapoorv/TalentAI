...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        if (isRecruiter) {
          // Recruiters: Notify about applicants for their jobs
          const jobsRes = await api.get('/jobs');
          const recruiterJobs = jobsRes.data;

          let newNotifications = [];
          for (const job of recruiterJobs.slice(0, 5)) {
            if (job.applicant_count > 0) {
              newNotifications.push({
                id: `job-${job._id}`,
                title: 'Applicants Received',
                text: `${job.applicant_count} candidates applied for ${job.role}`,
                icon: 'group',
                time: 'Recently'
              });
            }
          ...
          // Candidates: Notify about application updates
          const appsRes = await api.get('/applications/my');
          const myApps = appsRes.data;

          const newNotifications = myApps.slice(0, 5).map(app => ({
            id: app._id,
            title: 'Application Update',
            text: `Your application for ${app.job_role || 'Position'} is now ${app.status}`,
            icon: app.status === 'Shortlisted' ? 'verified' : 'info',
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-4 relative">
              ...
              >
                ...
                {notifications.length > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full border-2 border-white ring-2 ring-rose-500/10"></span>}
              </button>
              ...
              {showNotifications && (
                <div className="absolute top-14 right-0 w-80 bg-white border border-slate-100 rounded-[2rem] shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                    ...
                    <span className="text-[10px] bg-primary/10 text-primary px-3 py-1 rounded-full font-black uppercase tracking-widest">{notifications.length} New</span>
                  </div>
                  <div className="max-h-[400px] overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        <div key={n.id} className="p-5 border-b border-slate-50 hover:bg-slate-50/80 transition-all cursor-pointer group">
                          <div className="flex gap-4">
                            <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all duration-500 shadow-sm">
